...
/// Plain literals use [`StrError::Static`](crate::StrError::Static); formatted
/// messages use [`StrError::Owned`](crate::StrError::Owned). Storage is selected
...
/// `&'static str`. These messages use `StrError::Static` and keep their text unchanged:
/// ```
...
macro_rules! error {
    ...
    ($fmt:expr, $($arg:tt)*) => {
        $crate::ErrorUnion::new::<_, $crate::AnyError, _>($crate::StrError::Owned($crate::__private::format!($fmt, $($arg)*)))
    };
}
